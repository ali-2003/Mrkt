// schemas/device.js - UPDATED WITH BUSINESS PRICING
export default {
  name: 'device',
  title: 'Device',
  type: 'document',
  fields: [
    ...
    // 💰 UPDATED PRICING SECTION WITH BUSINESS PRICING
    {
      name: 'price',
      title: 'Regular Price',
      type: 'number',
      validation: Rule => Rule.required().min(0),
      description: 'Standard retail price for individual customers'
    },
    {
      name: 'sale_price',
      title: 'Sale Price',
      ...
      description: 'Discounted price for individual customers (optional). If set, this will be shown instead of regular price for non-business users.'
    },
    {
      name: 'business_price',
      ...
      type: 'number',
      validation: Rule => Rule.min(0),
      description: 'Wholesale price for business customers (optional). This will be shown to logged-in business accounts.'
    },
    {
      name: 'originalPrice',
      title: 'Original Price (Before Any Discounts)',
      type: 'number',
      validation: Rule => Rule.min(0),
      description: 'Original MSRP price (optional). Used to show crossed-out price when sale_price or business_price is active.'
    },
    {
      name: 'rating',
  ...
  preview: {
    ...
    prepare({ title, media, price, sale_price, business_price, deviceType, reviewCount, rating }) {
      const stars = rating ? `${rating}★` : 'No rating';
      const reviews = reviewCount ? `${reviewCount} reviews` : 'No reviews';
      
      // Enhanced preview with pricing info
      let priceDisplay = `Rp ${price?.toLocaleString('id-ID') || 'No price'}`;
      if (sale_price) {
        priceDisplay += ` (Sale: Rp ${sale_price.toLocaleString('id-ID')})`;
      }
      if (business_price) {
        priceDisplay += ` (Business: Rp ${business_price.toLocaleString('id-ID')})`;
      }
      
      return {
        title,
        media,
        subtitle: `${deviceType ? deviceType.toUpperCase() : ''} - ${priceDisplay} | ${stars} (${reviews})`
      }
    }
  }
}